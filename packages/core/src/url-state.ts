...
//
// Size strategy: the curated catalog is ~26KB of JSON; we never send it over
// the wire. Instead we diff the current WorkingTree against the base catalog
// passed in by the caller and transmit only (a) excluded curated ids, (b) per-id
// field overrides, (c) custom node subtrees. LZ-string compresses the diff JSON
// into a base64url-safe slug.
//
// Schema ownership: `v: 1` is bumped if we ever rename fields in a way that
// would confuse old share links. decode() returns null on unknown versions so
// the caller falls back to fresh state instead of silently loading garbage.
//
// Base-as-parameter: this module is framework-agnostic. The web app passes its
// bundled `catalog.json`; the CLI passes whatever the user resolved via `--base`.

import { compressToEncodedURIComponent, decompressFromEncodedURIComponent } from 'lz-string';

import type { Catalog, Language, Node, Probe, RootMeta, WorkingTree } from './types.js';
import { findNode, freshTree } from './tree.js';
...
export function encodeTree(tree: WorkingTree, base: Catalog): string {
  const diff = computeDiff(tree, base);
  return compressToEncodedURIComponent(JSON.stringify(diff));
}

export function decodeTree(s: string, base: Catalog): WorkingTree | null {
  try {
    const json = decompressFromEncodedURIComponent(s);
    if (!json) return null;
    const diff = JSON.parse(json) as TreeDiff;
    if (diff?.v !== 1) return null;
    return applyDiff(diff, base);
  } catch {
    return null;
  }
}

export function computeDiff(tree: WorkingTree, base: Catalog): TreeDiff {
  const diff: TreeDiff = { v: 1 };
  if (tree.language && tree.language !== 'en') diff.lang = tree.language;

  const root: Partial<RootMeta> = {};
  if (tree.root.probe !== base.root.probe) root.probe = tree.root.probe;
  if (tree.root.menu !== base.root.menu) root.menu = tree.root.menu;
  if (tree.root.title !== base.root.title) root.title = tree.root.title;
  if ((tree.root.remark ?? null) !== (base.root.remark ?? null)) {
  ...
  walkUserTree(tree.nodes, null, (node, parentId) => {
    ...
    const curated = findNode(base.nodes, node.id);
    if (curated) {
...
export function applyDiff(diff: TreeDiff, base: Catalog): WorkingTree {
  const tree = freshTree(base, diff.lang ?? 'en');
...
// Browser helpers (readHashState / writeHashState / buildShareUrl) moved to
// patch.ts so they can serve both v:1 (legacy TreeDiff JSON) and v:2
// (compressed patch YAML) formats from a single entry point.
