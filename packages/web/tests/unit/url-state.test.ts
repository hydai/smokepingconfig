...
import { applyDiff, computeDiff, decodeTree, encodeTree } from '@smokepingconf/core';

import { baseCatalog, findNode, freshTree, tree } from '../../src/lib/store.js';
...
describe('computeDiff / applyDiff', () => {
  it('fresh tree produces a minimal diff', () => {
    const d = computeDiff(freshTree(), baseCatalog);
  ...
  it('records language only when non-default', () => {
    ...
    expect(computeDiff(t, baseCatalog).lang).toBe('zh-TW');
    expect(computeDiff(freshTree('en'), baseCatalog).lang).toBeUndefined();
  });
  ...
  it('records probe removal as null', () => {
    ...
    delete dns.probe;
    const d = computeDiff(t, baseCatalog);
    expect(d.ov?.['c:DNSProbes']?.probe).toBeNull();
    const decoded = applyDiff(d, baseCatalog);
...
describe('encodeTree / decodeTree', () => {
  it('URL-safe round-trip', () => {
    ...
    const decoded = decodeTree(slug, baseCatalog);
    expect(decoded).toBeTruthy();
    expect(findNode(decoded!.nodes, 'c:CDN/Cloudflare')?.included).toBe(false);
  });

  it('rejects malformed slug', () => {
    expect(decodeTree('this is not a valid lz-string', baseCatalog)).toBeNull();
    expect(decodeTree('', baseCatalog)).toBeNull();
  });

  it('rejects wrong schema version', () => {
    // Sanity: a valid tree still encodes/decodes.
    ...
    expect(decodeTree(slug, baseCatalog)).toBeNull();
    expect(decodeTree(bad, baseCatalog)).toBeTruthy();
  });
...
describe('tree store integration — hash hydration', () => {
  it('freshTree matches applyDiff({v:1}) for default state', () => {
    const a = freshTree();
    const b = applyDiff({ v: 1 }, baseCatalog);
